...
export default async function DeveloperPostsPage() {
  const t = await getTranslations('developer');
  const igId = process.env.IG_DEVELOPER_ID!;
              ...
              <ScrollReveal key={post.id}>
                ...
                >
                  {img ? (
                    <Image
                      src={img}
                      alt={post.caption?.slice(0, 80) || 'Daho Developer'}
                      fill
                    ...
                    <div className="w-full h-full" style={{ background: 'linear-gradient(135deg, #6366f1 0%, #4338ca 100%)' }} />
                  )}
